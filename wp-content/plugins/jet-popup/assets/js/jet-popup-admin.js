		...
		initPopupLibrary: function() {
			...
			Vue.component( 'jet-popup-library-settings-manager', {
				...
				watch: {
					...
					onDateValueStart: function ( curr ) {
						this.settings['jet_popup_start_date_and_time'] = curr + ' ' + this.onDateTimeValueStart;
					},
					onDateTimeValueStart: function ( curr ) {
						let currTime = curr.length ? curr : '00:00';

						this.settings['jet_popup_start_date_and_time'] = this.onDateValueStart + ' ' + currTime;
					},

					onDateValueEnd: function ( curr ) {
						this.settings['jet_popup_end_date_and_time'] = curr + ' ' + this.onDateTimeValueEnd;
					},
					onDateTimeValueEnd: function ( curr ) {
						let currTime = curr.length ? curr : '00:00';

						this.settings['jet_popup_end_date_and_time'] = this.onDateValueEnd + ' ' + currTime;
					},
				...
				computed: {
					...
					triggerTypeCloseOptions: function () {
						return window.JetPopupLibraryConfig.popupCloseOptions || [];
					},
					...
					getPopupSettings: function () {
						...
						wp.apiFetch( {
							...
							if ( response.success ) {
								...
								if ( settings.hasOwnProperty( 'jet_popup_start_date_and_time' ) ) {
									let dateTimeDataStart = settings.jet_popup_start_date_and_time.split( ' ' );

									this.onDateValueStart = dateTimeDataStart[0] || '';
									this.onDateTimeValueStart = dateTimeDataStart[1] || '';
								}

								if ( settings.hasOwnProperty( 'jet_popup_end_date_and_time' ) ) {
									let dateTimeDataEnd = settings.jet_popup_end_date_and_time.split( ' ' );

									this.onDateValueEnd = dateTimeDataEnd[0] || '';
									this.onDateTimeValueEnd = dateTimeDataEnd[1] || '';
								}

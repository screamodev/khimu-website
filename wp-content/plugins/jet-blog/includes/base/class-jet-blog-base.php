...
abstract class Jet_Blog_Base extends Widget_Base {
	...
	public function _render_meta( $position = '', $base = '', $context = array( 'before' ), $settings = array() ) {
		...
		foreach ( $meta_config as $meta ) {
			...
			? sprintf( '<div class="%1$s__item-icon">%2$s</div>', $base, $icon )
			: '';

			$meta_image = '';

			if ( ! empty( $meta['item_image']['url'] ) && $meta_val ) {
				$meta_image = sprintf( '<img class="%1$s__item-image" src="%2$s" alt="">', $base, $meta['item_image']['url'] );
			}
		...
		printf( '<div class="%1$s">%2$s</div>', $base, stripcslashes( $result ) );
